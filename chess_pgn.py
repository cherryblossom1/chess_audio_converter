...
class GameNode(abc.ABC):
    ...
    def eval(self) -> Optional[chess_engine.PovScore]:
        """
        ...
        if match.group(1):
            ...
            score: chess_engine.Score = chess_engine.Mate(mate)
            if mate == 0:
                # Resolve this ambiguity in the specification in favor of
                # standard chess: The player to move after mate is the player
                # who has been mated.
                return chess_engine.PovScore(score, turn)
        else:
            score = chess_engine.Cp(int(float(match.group(2)) * 100))

        return chess_engine.PovScore(score if turn else -score, turn)
    ...
    def set_eval(self, score: Optional[chess_engine.PovScore], depth: Optional[int] = None) -> None:
        """
    ...
    def arrows(self) -> List[chess_svg.Arrow]:
        """
        Parses all ``[%csl ...]`` and ``[%cal ...]`` annotations in the comment
        of this node.

        Returns a list of :class:`arrows <chess_svg.Arrow>`.
        """
        arrows = []
        for match in ARROWS_REGEX.finditer(self.comment):
            for group in match.group(1).split(","):
                arrows.append(chess_svg.Arrow.from_pgn(group))

        return arrows



    def set_arrows(self, arrows: Iterable[Union[chess_svg.Arrow, Tuple[Square, Square]]]) -> None:
        """
        ...
        for arrow in arrows:
            try:
                ...
                arrow = chess_svg.Arrow(tail, head)
            except TypeError:
                pass
